...
def list_allocations(limit: int = 100, offset: int = 0, db: Session = Depends(get_db)):
...
def create_allocation(allocation_data: PaymentAllocationCreate, db: Session = Depends(get_db)):
...
def update_allocation(
    allocation_id: int, allocation_data: PaymentAllocationUpdate, db: Session = Depends(get_db)
):
