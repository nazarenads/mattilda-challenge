...
class TestSchoolList:
    ...
    def test_list_schools_with_data(self, client, db_helpers):
    ...
    def test_list_schools_pagination(self, client, db_helpers):
        for i in range(5):
        ...
        response_page2 = client.get("/school/?limit=2&offset=2")
...
class TestSchoolCreate:
    def test_create_school(self, client, db_helpers):
        school_data = {
            "name": "Test School",
            "country": "US",
            "tax_id": "123456789"
...
class TestSchoolGet:
    def test_get_school(self, client, db_helpers):
...
class TestSchoolUpdate:
    def test_update_school(self, client, db_helpers):
        ...
        response = client.put(f"/school/{school.id}", json=update_data)
...
class TestSchoolDelete:
    def test_delete_school(self, client, db_helpers):
...
class TestSchoolBalance:
    def test_get_school_balance_empty(self, client, db_helpers):
        school = db_helpers.create_school()

        response = client.get(f"/school/{school.id}/balance")
    ...
    def test_get_school_balance_with_data(self, client, db_helpers):
        school = db_helpers.create_school()
    ...
    def test_get_school_balance_not_found(self, client):
        response = client.get("/school/999/balance")

        assert response.status_code == 404
        assert response.json()["detail"] == "School not found"

    def test_get_school_balance_excludes_pending_payments(self, client, db_helpers):
        school = db_helpers.create_school()
